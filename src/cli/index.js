...
program
	...
	.description('Start the application server and view the live output log')
	.action(function () {
		program.log = true;
		require('./running').start(program);
	});
program
	...
	.description('Start application in verbose development mode')
	.action(function () {
		program.dev = true;
...
program
	.command('status')
	.description('Check the running status of the application server')
	.action(function () {
...
program
	...
	.description('Run the application setup script, or setup with an initial config')
...
program
	.command('install')
	.description('Launch the application web installer for configuration setup')
	.action(function () {
		require('./setup').webInstall();
	});
program
...
program
	...
	.description('Activate a plugin for the next startup of application (nodebb-plugin- prefix is optional)')
...
program
	...
	.description('Outputs the most recent administrative events recorded by application')
...
program
	...
	.description('Run application upgrade scripts and ensure packages are up-to-date, or run a particular upgrade script')
	...
	.option('-s, --schema', 'Update application data store schema', false)
	...
	.on('--help', function () {
		console.log('\n' + [
			...
			'Only package and dependency updates: ' + './node upgrade -mi'.yellow,
			'Only database update: ' + './node upgrade -s'.yellow,
		].join('\n'));
	})
