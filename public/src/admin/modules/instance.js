...
define('admin/modules/instance', [
	...
	instance.rebuildAndRestart = function (callback) {
		app.alert({
			...
			message: 'Application is rebuilding front-end assets (css, javascript, etc).',
		});
		...
		$(window).one('action:reconnected', function () {
			app.alert({
				...
				message: 'Application has rebuilt and restarted successfully.',
				timeout: 5000,
			});
	...
	instance.restart = function (callback) {
		...
		$(window).one('action:reconnected', function () {
			app.alert({
				...
				type: 'success',
				title: '<i class="fa fa-check"></i> Success',
				message: 'Application has restarted successfully.',
				timeout: 5000,
			});
